...
import { FTM_TICKER, ECTO_TICKER, ESHARE_TICKER, ZAPPER_ROUTER_ADDR } from '../utils/constants';
...
function useApproveZapper(zappingToken: string): [ApprovalState, () => Promise<void>] {
  ...
  let token: ERC20;
  if (zappingToken === FTM_TICKER) token = ectoFinance.FTM;
  else if (zappingToken === ECTO_TICKER) token = ectoFinance.ECTO;
  else if (zappingToken === ESHARE_TICKER) token = ectoFinance.ESHARE;
  ...
  const approvalState: ApprovalState = useMemo(() => {
    ...
    if (token === ectoFinance.FTM) return ApprovalState.APPROVED;
  ...
  }, [currentAllowance, pendingApproval, token, ectoFinance]);

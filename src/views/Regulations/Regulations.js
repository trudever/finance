...
const StyledTableCell = withStyles((theme) => ({
  head: {
    fontSize: 18,
    backgroundColor: 'rgba(52, 190, 130, 0.9)',
    color: '#001E6C',
    fontWeight: 'bolder',
  },
  body: {
...
const Regulations = () => {
  ...
  useEffect(() => {
    if (ectoFinance) {
      const thisData = ectoFinance.listenForRegulationsEvents();
          ...
          <TableBody>
            {rows?.map((row, index) => (
              <StyledTableRow
                ...
                key={row.epoch}
              >
                <StyledTableCell style={{ color: '#001E6C' }} align="center" component="th" scope="row">
                  {row.epoch}
                </StyledTableCell>
